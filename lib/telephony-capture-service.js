...
(function (TelephonyCaptureService) {
    ...
    const isTmsEnabled = process.argv[3] === "true" ? true : false;
    ...
    setTimeout(() => {
        if (isTmsEnabled) {
            tmsQueue = new queue_1.Queue(constants_1.TMS_QUEUE, dockerMachineIp, null);
        }
        databaseQueue = new queue_1.Queue(constants_1.DATABASE_QUEUE, dockerMachineIp, null);
