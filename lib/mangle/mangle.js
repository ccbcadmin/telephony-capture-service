...
process.on("SIGTERM", () => {
    Barrel_1.trace("Telephony Capture Service: Terminated");
    process.exit(0);
});
process.on("SIGINT", () => {
    Barrel_1.trace("Telephony Capture Service: Ctrl-C received. Telephony Capture Service terminating");
    process.exit(0);
});
let smdrFiles = [];
let smdrFileNo = 0;
...
const nextFile = () => {
    if (smdrFileNo === smdrFiles.length) {
        Barrel_1.trace(`That's All Folks !`);
        process.exit(0);
    }
    else {
...
dir.files(sourceDir, (err, files) => {
    if (err) {
        Barrel_1.trace(`Source ${sourceDir} is not a directory...aborting.`);
        process.exit(1);
    }
    files.sort();
