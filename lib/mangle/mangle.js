...
const replicateSmdrFile = (smdrFileName) => {
    ...
    let filePart = smdrFileName.split(util_1.pathSeparator());
    ...
    const outputPath = [env.SMDR_DATA, env.MANGLE_TARGET_DIRECTORY, outputFile].join(util_1.pathSeparator());
...
dir.files(env.MANGLE_SOURCE_DIRECTORY, (err, files) => {
    ...
    for (let file of files) {
        let path = file.split(util_1.pathSeparator());
