...
class ServerSocket {
    constructor(linkName, port, dataSink, linkCloseHandler = undefined) {
        ...
        this.handleConnection = (connection) => {
            ...
            Barrel_1.trace(`${this.linkName}: Connection Open: ${remoteAddress}`);
            const onClose = () => {
                Barrel_1.trace(`${this.linkName}: Connection Closed: from ${remoteAddress}`);
                connection.removeListener("data", this.dataSink);
                connection.removeListener("error", onError);
            };
            const onError = (err) => {
                Barrel_1.trace(`${this.linkName}: Connection ${remoteAddress} error: ${err.message}`);
            };
        ...
        this.startListening = () => {
            ...
            this.server.listen({
                ...
                Barrel_1.trace(`${this.linkName}: Listening on: ${this.port}`);
            });
        };
        this.stopListening = () => {
            Barrel_1.trace(`${this.linkName}: Stop Listening`);
